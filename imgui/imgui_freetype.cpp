...
namespace 
{
    ...
    bool FreeTypeFont::Init(const ImFontConfig& cfg, unsigned int extra_user_flags)
    {
        ...
        if (error != 0)
            return false;
      /*  
        error = FT_Library_SetLcdFilter(FreetypeLibrary, FT_LCD_FILTER_DEFAULT);
        if (error)
            return false;
            */
       ...
       // FreetypeLoadFlags |= FT_LOAD_TARGET_LCD;
    ...
    for (int input_i = 0; input_i < atlas->ConfigData.Size; input_i++)
    {
        ...
        for (const ImWchar* in_range = cfg.GlyphRanges; in_range[0] && in_range[1]; in_range += 2) 
        {
            for (uint32_t codepoint = in_range[0]; codepoint <= in_range[1]; ++codepoint) 
            {
                ...
                //BlitGlyphLCD(&ft_glyph_bitmap->bitmap, 0, 0, blit_dst, atlas->TexWidth, atlas->TexHeight);
